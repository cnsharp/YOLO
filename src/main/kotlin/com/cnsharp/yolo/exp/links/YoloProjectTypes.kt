 ...
 *
 * IMPORTANT — [snapshot] must never block. It is called from `Filter.applyFilter` for every rendered line,
 * so a read action there would stall terminal output. [snapshot] therefore returns the last computed value
 * immediately and (re)computes it on a pooled thread under a read action when the PSI/index has changed;
 * until the refresh finishes, the previous (or empty) snapshot is returned.
 */
object YoloProjectTypes {
     ...
     * first build, empty — snapshot is returned. Safe to call from the filter's rendering path.
     */
    fun snapshot(project: Project): Snapshot {
        ...
        if (cache.modCount != modCount && cache.refreshScheduled.compareAndSet(false, true)) {
            ApplicationManager.getApplication().executeOnPooledThread {
                try {
                    // We're on a pooled thread, so a blocking (non-cancellable) read action is correct.
                    val built = runReadActionBlocking { build(project) }
 ...
 * `getItemsByName(name, name, project, false)` — the same project-only check [resolveType] uses on click.
 * This second pass is mandatory: `getNames(project, false)` is SCOPE-BLIND for several language
 * contributors (Java's `AllClassesContributor` returns every indexed class short name — project +
