...
/**
 * Exercises [StackTraceLinkFilter.applyFilter] — the pure link-matching logic — against the reported
 ...
 * `project` for the deferred click handler, so filtering is fully testable with a `null` project (the
 * link's `HyperlinkInfo` is constructed but never executed here). This keeps `:test` offline — it reuses
 * the local IDEA platform classpath used for compilation instead of the plugin's `testFramework(...)`
 * helper.
 */
class StackTraceLinkFilterTest {
    ...
    fun testWrapTailSuppressedWhenContinuationSpanSet() {
        ...
        val line = "        rTransitionContext.java"
        assertTrue(filter.applyFilter(line, line.length).let {
            it == null || it.resultItems.isEmpty()
        })
    }
